...
| :------ | :---------- | :----------- | :----------- | :----------- |
| **BashProfilePersist** |  Modifies user's bash profile to execute script if the persistence process (current implementation assumes osascript) is not already running. If Catalina system then .zshenv is modified. <br /> Persistence executes on terminal open. | js_importcall BashProfilePersist('osascript -l JavaScript -e ...', "no") |  $HOME/.bash_profile or  $HOME/.zshenv <br /> <br /> *If select "yes" for hidden file creation then:* $HOME/.security/apple.sh <br />  $HOME/.security/update.sh <br /> | *If select "yes" for hidden file creation then:* <br /> sh $HOME/.security/apple.sh <br /> <br /> sh $HOME/.security/persist.sh|
